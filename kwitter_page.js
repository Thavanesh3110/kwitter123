var firebaseConfig = {
    ...
    function send(){
      msg = document.getElementById("user_name").value;
...
function getData() { firebase.database().ref("/"+room_name).on('value', function(snapshot) { document.getElementById("output").innerHTML = ""; snapshot.forEach(function(childSnapshot) { childKey  = childSnapshot.key; childData = childSnapshot.val(); if(childKey != "purpose") {
      ...
      button_with_tag = "<button class='btn btn-success' id="+firebase_message_id+" value="+like+" onclick='update_like(this.id)'>";
...
function update_like(message1){
      console.log(message1);
      button_id = message1;
      likes = document.getElementById(button_id).value;
      updated_likes = Number(likes) + 1;
      firebase.database().ref(room_name).child(message1).update({
            like : updated_likes
      });
}
function logout(){
